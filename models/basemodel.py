...
class BaseModel():
    ...
    def save_weights(self, epoch:int, is_best:bool=False):
        ...
        weight_dir = os.path.join(
            self.opt.outfolder, self.opt.name, 'train', 'weights')
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            loss = 'gloss: ' + str(self.err_g.item()) + ' dloss:' + str(self.err_d.item())
            print(loss)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.data.valid, 0):
                ...
                if self.opt.save_test_images:
                    dst = os.path.join(self.opt.outfolder, self.opt.name, 'test', 'images')
